...
export default class BaseComponent {
  ...
  protected __doc() {
    ...
    if (fs.existsSync(docPath)) {
      ...
      const rows = [];
      const data = get(result, 'children[0].children', []).filter(
        (item) => item.kindString === 'Method' && get(item, 'flags.isPublic'),
      );
      ...
      data.forEach((item) => {
        ...
        rows.push([
          item.name,
          get(item, 'signatures[0].comment.shortText', ''),
          parmasThat,
          `s cli ${this.name} ${item.name} ${
            paramText.length > 1 ? '--props ' + paramText : ''
          } -a default -r cn-hangzhou`,
        ]);
      });
  ...
  protected __listApi() {
    ...
    if (fs.existsSync(docPath)) {
      ...
      const data = result.children[0].children.filter(
  ...
  protected checkField(filed: {}): boolean {
    ...
    for (var key in filed) {
      ...
      if (flag) {
        this.logger.error(
          `Please check the parameters. These parameters are required: ${String(
            Object.keys(filed),
          )}.`,
        );
        process.exit(-1);
